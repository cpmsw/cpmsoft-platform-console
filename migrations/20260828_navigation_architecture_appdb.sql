...
-- Database: appdb
-- Purpose:
--   Backfill role permissions for the new navigation/core module resources.
--
-- Important:
--   Resource UUIDs are stable and are created by the matching authdb migration.
--
-- Safe re-run behavior:
--   Permission backfill runs only once, guarded by schema_migrations,
--   so later administrator permission changes are not silently restored.
...
WHERE r.is_active = true
  AND r.role_code IN ('PRIMARY', 'ADMIN')
  AND p.is_active = true
  AND p.permission_key IN
      ('view', 'create', 'edit', 'deactivate', 'history_view')
  AND NOT EXISTS
  (
      SELECT 1
      ...
      WHERE rp.tenant_id = r.tenant_id
        AND rp.role_id = r.id
        AND rp.resource_id = nr.resource_id
...
CROSS JOIN new_resources nr
JOIN permissions p
    ON p.permission_key IN ('view', 'create', 'edit')
   AND p.is_active = true
WHERE r.is_active = true
...
JOIN roles r
    ON r.tenant_id = pt.tenant_id
   AND r.is_active = true
CROSS JOIN permissions p
WHERE
    (
        r.role_code IN ('PRIMARY', 'ADMIN')
        AND p.permission_key IN
            ('view', 'create', 'edit', 'deactivate', 'history_view')
    )
    OR
